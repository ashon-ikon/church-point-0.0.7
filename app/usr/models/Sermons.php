...
class Point_Model_Sermons
{
	 ...
	 * Sermon Authors
	 */
	protected	$_sermon_authors = null;
	...
	public function getContentGroupId()
	{
		if (null === $this->_content_group_id)
		{
			...
			$this->_content_group_id = $contentGroups->getGroupIdByKeyword('Sermon');
	...
	public function getSermonAuthorsInstance()
	{
		if (null === $this->_sermon_authors)
		{
			$this->_sermon_authors = Point_Model_Sermons_Authors::getInstance();
		}
		return $this->_sermon_authors;
	}
	...
	{
		$authors_obj	= $this->getSermonAuthorsInstance();
		return $authors_obj->addSpeaker($firstname, $lastname, $email, $image);
	}

	/**
	 * Remove speaker profile
	 * 
	 * @abstract	This removes a speaker from speakers list
	 * 
	 * @param int	author_id
	 * 
	 * @return boolean success | failure
	 */
	public function removeSpeaker ( $speaker_id )
	{
		if (!is_numeric($speaker_id))
			throw new Exception('Speaker ID must be numeric');
			
		$authors_obj	= $this->getSermonAuthorsInstance();
		return $authors_obj->removeSpeaker($speaker_id);
	}
	
	/**
	 * Updates new speaker profile
	 * 
	 * @param int 	 Author ID
	 * @param string firstname
	 * @param string lastname
	...
	{
		$authors_obj	= $this->getSermonAuthorsInstance();
		return $authors_obj->updateSpeaker($id, $firstname, $lastname, $email, $image);
	}
	 ...
	 * 
	 */
	public function getSpeakers($start = 0, $amount = 10, $order = 'ASC')
	{
		$authors_obj	= $this->getSermonAuthorsInstance();
		return $authors_obj->getAllSpeakers($start, $amount, $order);
	}
...
//					  ->joinLeft(array('vt' =>'videos_table'), 'st.sermon_video_id = at.video_id');
	...
	{
		$like 	=	$id	= $precise 	= $year = $month	= $day	= $title	=  $file  	= null;
		...
		if (is_numeric($id))
		{
			$like	= $this->getDbTable()->getAdapter()->quoteInto('sermon_id = ?', $id);
		}
		else if (!$year)
		{
...
//					  ->joinLeft(array('vt' =>'videos_table'), 'st.sermon_video_id = at.video_id');
	...
	/**
	 * This function gets the last sermon by speaker
	 */
	public function getLastSermonBySpeaker($id)
	{
		if (!is_numeric($id))
			throw new Exception ('Sermon Speaker ID must be numeric');
			
		$db			= $this->getDbTable();
		$lastSermon	= $db->select()->where('sermon_author_id = ?', $id, 'INT')->order('sermon_date DESC')->limit(1)->query()->fetch();
	...
	/**
	 * This function gets all sermons by speaker
	 */
	public function getAllSermonBySpeaker($id)
	{
		if (!is_numeric($id))
			throw new Exception ('Sermon Speaker ID must be numeric');
			
		$db			= $this->getDbTable();
		$sermons	= $db->select()->where('sermon_author_id = ?', $id, 'INT')->order('sermon_date DESC')->query()->fetchAll();
	...
	public function getSermonSpeaker($id)
	{
		...
		$speakers_obj	= $this->getSermonAuthorsInstance();
		$speaker 	= $speakers_obj->getSpeakerById($id);
