...
class Point_Object_Menus
{
	 ...
	 */
	public function getMenus($asArray = false, array $tags = array())
	{
		...
		else
		{
			...
			if (!$parent_tag 	= getArrayVar($tags, 'parent'))
				 $parent_tag 	= 'ul';
			
			if (!$child_tag 	= getArrayVar($tags, 'child'))
				 $child_tag 	= 'li';
			...
			{
				$html 	.= $this->_makeMenu($menu, $parent_tag, $child_tag) . "\n";
			}
	 ...
	 */
	protected function _makeMenu($menu, $parent_tag, $child_tag)
	{
		$view 	= $this->_getView();
		$ret	= null;
		
		$link	= '<a href="'. $view->fullUrl(array('controller' => $menu['page_controller'],
												 'action'		=> $menu['page_action']), null , true) .'" title="'. $view->escape($menu['page_title']).'">'.
												 $view->escape($menu['page_name']) .'</a>';
		if ($child_menus = getArrayVar($menu, 'children'))
		{
			...
			{
				$ret.= $this->_makeMenu($child_menu, $parent_tag, $child_tag). "\n";	
			}
